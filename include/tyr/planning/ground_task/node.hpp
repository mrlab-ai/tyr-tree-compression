...
public:
    ...
    Node(StateView<GroundTask> state, float_t metric) noexcept : m_state(std::move(state)), m_metric(metric) {}

    const StateView<GroundTask>& get_state() const noexcept { return m_state; }
